...
function GenererOccurrences(debut, fin, parents) {
  const occurences = [];
  if(!parents || parents.length == 0) return occurences;
  for (const parent of parents) {
    const duree = DateTime.fromSQL(parent.fin,  {zone:'utc'}).diff(
      DateTime.fromSQL(parent.debut,  {zone:'utc'})
    );
    //console.log('GenererOccurences parent debut', parent.debut, 'fin', parent.fin, 'regle', parent.regle_recurrence, 'duree', duree)
